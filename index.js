...
let arrCharacter = '';
function test(args, character) {
    const checks = document.getElementById(`check${args}`);
    checks === null || checks === void 0 ? void 0 : checks.addEventListener('click', () => {
        if (checks.checked) {
...
function makePassword(length = arrCharacter.length) {
    const randomPosition = (min = 0, max = length) => {
        return Math.round(Math.random() * (max - min) + min);
    };
    let password = '';
    if (arrCharacter.length > 0) {
        for (let i = ''; i.length != 10; i = i.concat(arrCharacter.charAt(randomPosition()))) {
            password = i;
        }
        console.log(password);
    }
...
(_a = document
    .getElementById('makePassord')) === null || _a === void 0 ? void 0 : _a.addEventListener('click', () => makePassword());
